...
| **v1 ships with no print/export path for the cutting plan** — the operator's only way to reference it is on-screen (Section 3). This is a **pre-existing consequence of the Phase F deferral** (Section 10, already decided before this document's Section 7 addition) — not a new risk, not a new product/architectural/scope decision, and not a new Open Question; Section 8 simply had not documented it until now | UX / Product | Operator relies on the on-screen Optimization Results/Execution Summary for v1, consistent with Section 6's "plan for a human to follow" assumption; Phase F's already-scheduled `ExportGenerator`-based cutting-sheet PDF (Section 7, Section 10) becomes the mitigation once scheduled — not a blocker for Phases A–E |
| **Cut Optimizer implementation is fully blocked pending Open Question 4** — Phase A (and therefore every subsequent phase, per Section 10's sequential dependencies) cannot begin until the minimum-useful-offcut threshold question is formally resolved (Strategy D, Section 11) — not a new risk, not a new product/architectural/scope decision, and not a new Open Question; Section 8 simply had not documented this already-decided consequence until now | Product / Schedule | This is an intentional, deliberate Product Owner choice (Section 11) to avoid introducing any unapproved interim default or placeholder value for `exceedsUsefulThreshold` into Phase A — not a technical mitigation, an accepted schedule consequence; Phase A resumes as soon as Open Question 4 is resolved |
